...
public class StorageTest {
    ...
    public void setUp() {
        fTest_ = new FormatsTest();
    }

    @Test
    public void testPolicies() {
        Assertions.assertEquals( StoragePolicy.PREFER_MEMORY, getPolicy( "memory" ) );
        Assertions.assertEquals( StoragePolicy.PREFER_DISK, getPolicy( "disk" ) );
        Assertions.assertEquals( StoragePolicy.SIDEWAYS, getPolicy( "sideways" ) );
        Assertions.assertEquals( StoragePolicy.DISCARD, getPolicy( "discard" ) );
        Assertions.assertEquals( StoragePolicy.ADAPTIVE, getPolicy( "adaptive" ) );

        Assertions.assertEquals( "StoragePolicy.PREFER_MEMORY",
                       StoragePolicy.PREFER_MEMORY.toString() );
        Assertions.assertEquals( "StoragePolicy.PREFER_DISK",
                       StoragePolicy.PREFER_DISK.toString() );
        Assertions.assertEquals( "StoragePolicy.SIDEWAYS",
                       StoragePolicy.SIDEWAYS.toString() );
        Assertions.assertEquals( "StoragePolicy.DISCARD",
                       StoragePolicy.DISCARD.toString() );
        Assertions.assertEquals( "StoragePolicy.ADAPTIVE",
    ...
    public void testLimitByteStore() throws IOException {
        ...
        Assertions.assertEquals( 15, lbs.getLength() );
        try {
    ...
    private void testByteStore( ByteStore bs ) throws IOException {
        ...
        out.write( buf1 );
        Assertions.assertEquals( 333, bs.getLength() );
        out.write( buf, 333, 333 );
        Assertions.assertEquals( 666, bs.getLength() );
        for ( int i = 0; i < 333; i++ ) {
            Assertions.assertEquals( 666 + i, bs.getLength() );
            out.write( buf[ 666 + i ] );
        }
        Assertions.assertEquals( 999, bs.getLength() );
        out.flush();
        Assertions.assertEquals( 999, bs.getLength() );
    ...
    private void checkTables( StarTable tab1, StarTable tab2, StarTable tab3,
                              StarTable tab4 )
            throws IOException {
        Assertions.assertTrue( tab1.isRandom() );
        Assertions.assertTrue( tab2.isRandom() );
        Assertions.assertTrue( tab3.isRandom() );
        Assertions.assertTrue( tab4.isRandom() );
