...
import static uk.ac.starlink.util.TestUtils.fillRandom;
    ...
    public void testTiles() throws IOException {
        for ( Iterator it = ndaIterator(); it.hasNext(); ) {
            ...
            for ( Iterator tit = tileIterator( shape1 ); tit.hasNext(); ) {
                ...
                assertArrayEquals(new Object[]{buf1}, new Object[]{buf2} );
            }
            nda1.close();
        }
    }
    ...
    public void testMoulded() throws IOException {
        for ( Iterator it = ndaIterator(); it.hasNext(); ) {
            ...
            for ( int j = 0; j < npix; j++ ) {
                ...
                assertArrayEquals((long[]) it1.next(), p1 );
                assertArrayEquals((long[]) it2.next(), p2 );
                acc1.read( a1, j, 1 );
                acc2.read( a2, j, 1 );
            }
    ...
    public void testWindow() throws IOException {
        for ( Iterator it = ndaIterator(); it.hasNext(); ) {
            ...
            for ( Iterator tit = tileIterator( shape1 ); tit.hasNext(); ) {
                ...
                assertArrayEquals( (Object[]) buf1, (Object[]) buf2 );
            }
            nda1.close();
        }
    }
