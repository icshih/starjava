...
import static uk.ac.starlink.hdx.DOMComparer.assertDOMEquals;


public class HdxTest {

    static SourceReader srcrdr;
    static HdxFactory fact;
    static final Logger logger = LoggerFactory.getLogger(HdxTest.class);

    @BeforeAll
    public static void setUp() throws HdxException, ParserConfigurationException {
    ...
    public void testURIResolution() throws Exception {
        ...
        assertFalse(de instanceof HdxNode);
    ...
    @Test
    @Disabled
    public void testHdxDocumentImportNode() throws Exception {
        String xmlin
    ...
    public void testObjectOverrides() 
        ...
        assertNotSame(in, inclone);
        assertSame(inclone.getClass(), in.getClass());

        assertEquals(in, in); // reflexive
        assertEquals(inclone, in);
        assertEquals(in, inclone); // symmetric
        assertEquals(in, inclone); // consistent
        assertNotNull(in); // inequivalence with null
        assertEquals(inclone, clone2);
        assertEquals(in, clone2); // transitive
    ...
    private void assertNodeNameSequence(String[] s, Node n) {
        Iterator si = java.util.Arrays.asList(s).iterator();
        while (true) {
